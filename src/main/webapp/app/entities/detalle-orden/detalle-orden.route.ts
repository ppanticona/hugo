import { Injectable } from '@angular/core';
import { HttpResponse } from '@angular/common/http';
import { Resolve, ActivatedRouteSnapshot, Routes, Router } from '@angular/router';
import { Observable, of, EMPTY } from 'rxjs';
import { flatMap } from 'rxjs/operators';

import { Authority } from 'app/shared/constants/authority.constants';
import { UserRouteAccessService } from 'app/core/auth/user-route-access-service';
import { IDetalleOrden, DetalleOrden } from 'app/shared/model/detalle-orden.model';
import { DetalleOrdenService } from './detalle-orden.service';
import { DetalleOrdenComponent } from './detalle-orden.component';
import { DetalleOrdenDetailComponent } from './detalle-orden-detail.component';
import { DetalleOrdenUpdateComponent } from './detalle-orden-update.component';

@Injectable({ providedIn: 'root' })
export class DetalleOrdenResolve implements Resolve<IDetalleOrden> {
  constructor(private service: DetalleOrdenService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IDetalleOrden> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((detalleOrden: HttpResponse<DetalleOrden>) => {
          if (detalleOrden.body) {
            return of(detalleOrden.body);
          } else {
            this.router.navigate(['404']);
            return EMPTY;
          }
        })
      );
    }
    return of(new DetalleOrden());
  }
}

export const detalleOrdenRoute: Routes = [
  {
    path: '',
    component: DetalleOrdenComponent,
    data: {
      authorities: [Authority.USER],
      pageTitle: 'DetalleOrdens',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: ':id/view',
    component: DetalleOrdenDetailComponent,
    resolve: {
      detalleOrden: DetalleOrdenResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'DetalleOrdens',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: 'new',
    component: DetalleOrdenUpdateComponent,
    resolve: {
      detalleOrden: DetalleOrdenResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'DetalleOrdens',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: ':id/edit',
    component: DetalleOrdenUpdateComponent,
    resolve: {
      detalleOrden: DetalleOrdenResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'DetalleOrdens',
    },
    canActivate: [UserRouteAccessService],
  },
];
